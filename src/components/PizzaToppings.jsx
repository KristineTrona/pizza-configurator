      ...
      <div className = "topping-1">
        <label htmlFor="topping-selector-1">Topping 1 :</label>
          <select id = "topping-selector-1" name="topping-1" onChange={props.onChange}>
            <option key="" value = ""> Select topping</option>
            {props.toppings.map(topping => <option key= {topping.title} value={topping.title}>{topping.title} 	&euro; {0.50.toFixed(2)} </option>)}
          </select>
      </div>
